...
Citizen.CreateThread(function()
	...
	for k,v in pairs(Config.CoffeeShops.Parking.Blips) do
...
Citizen.CreateThread(function()
	...
	if PlayerData.job ~= nil and PlayerData.gang ~= nil and PlayerData.job.name == 'coffee' or PlayerData.gang.name == gangName then

		local playerPed = PlayerPedId()
		local coords    = GetEntityCoords(playerPed)

		for k,v in pairs(Config.CoffeeShops) do
		...
		if PlayerData.job.name == "coffee" then
			...
			if PlayerData.job ~= nil and PlayerData.job.name == 'coffee' and PlayerData.job.grade_name == 'boss' then
		...
		if PlayerData.gang ~= nil and PlayerData.gang.name == gangName then
			for i=1, #v.Bars, 1 do
...
Citizen.CreateThread(function()
	...
	if PlayerData.job ~= nil and PlayerData.gang ~= nil and PlayerData.job.name == 'coffee' or PlayerData.gang.name == gangName then
		...
		then
			TriggerEvent('esx_cafeshop:hasExitedMarker', LastStation, LastPart, LastPartNum)
			hasExited = true
		end
		...
		TriggerEvent('esx_cafeshop:hasEnteredMarker', currentStation, currentPart, currentPartNum)
		end
...
AddEventHandler('esx_cafeshop:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)

AddEventHandler('esx_cafeshop:hasEnteredMarker', function(station, part, partNum)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and (PlayerData.job.name == 'coffee' or PlayerData.gang.name == gangName) then
					if CurrentAction == "menu_refrigrator" then
						OpenGetRefrigrator()
					elseif CurrentAction == "menu_boss_actions" then

						TriggerEvent('esx_society:openBosirpixelsMenu', 'coffee', function(data, menu)
							menu.close()
						end, { wash = false }) -- disable washing money

					elseif CurrentAction == "menu_cloakroom" then
						OpenCloakroomMenu()
					elseif CurrentAction == "menu_bar_actions" then
						OpenBar()
					end
...
function OpenGetRefrigrator()
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			TriggerServerEvent("esx_cafeshop:giveItem", itemName)
