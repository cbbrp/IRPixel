...
Citizen.CreateThread(function()
        ...
        while true do
            ...
            if IsControlJustPressed(1, KeyOpenClose) and GetLastInputMethod( 0 ) then
                ESX.TriggerServerCallback('gcphone:doeshavephone', function(phone)

                    if phone then
                        
                        if not IsPedArmed(GetPlayerPed(-1), 7) then
                            ESX.TriggerServerCallback('gcphone:getbank', function(amount)
...
AddEventHandler("gcPhone:receiveMessage", function(message, havephone)
...
AddEventHandler("gcPhone:acceptCall", function(infoCall, initiator)
    if inCall == false and USE_RTC == false then
        inCall = true
        exports["mumble_voip"]:SetCallChannel(infoCall.id + 1)
...
AddEventHandler("gcPhone:rejectCall", function(infoCall, caller)
    if inCall == true then
        inCall = false
        exports["mumble_voip"]:SetCallChannel(0)
...
RegisterNetEvent('gcphone:phoneRemoved')
AddEventHandler('gcphone:phoneRemoved', function()
    SendNUIMessage({show = false})
    PhonePlayOut()
end)
