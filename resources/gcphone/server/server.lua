...
function getPhoneRandomNumber()
    ...
    local num = string.format("0939%03d%04d", numBase0, numBase1 )
	return num
end
...
RegisterServerEvent('gcPhone:_internalAddMessageirpixel')
AddEventHandler('gcPhone:_internalAddMessageirpixel', function(transmitter, receiver, message, owner, cb)
		...
		getSourceFromIdentifier(otherIdentifier, function (osou)
			if tonumber(osou) ~= nil then
				...
				if xPlayer then
					...
					TriggerClientEvent("gcPhone:receiveMessage", xPlayer.source , tomess, havephone)
				end
...
AddEventHandler('gcPhone:rejectCall', function (infoCall)
	...
	if AppelsEnCours[id] ~= nil then
		...
		if AppelsEnCours[id].transmitter_src ~= nil then
			TriggerClientEvent('gcPhone:rejectCall', AppelsEnCours[id].transmitter_src, false, AppelsEnCours[id].transmitter_src)
		end
		if AppelsEnCours[id].receiver_src ~= nil then
			TriggerClientEvent('gcPhone:rejectCall', AppelsEnCours[id].receiver_src, false, AppelsEnCours[id].transmitter_src)
		end
...
function onRejectFixePhone(source, infoCall, rtcAnswer)
	...
	PhoneFixeInfo[id] = nil
	TriggerClientEvent('gcPhone:notifyFixePhoneChange', -1, PhoneFixeInfo)
	TriggerClientEvent('gcPhone:rejectCall', AppelsEnCours[id].transmitter_src, source)
	if AppelsEnCours[id].is_accepts == false then
		saveAppels(AppelsEnCours[id])
	end
	AppelsEnCours[id] = nil
...
AddEventHandler('esx:onRemoveInventoryItem', function(source, item, count)
	if item.name == 'phone' and item.count < 1 then
		TriggerClientEvent('gcphone:phoneRemoved', source)
	end
end)
