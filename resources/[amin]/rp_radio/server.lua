...
AddEventHandler('esx:onRemoveInventoryItem', function(source, item, count)
	if item.name == 'radio' and item.count < 1 then
		TriggerClientEvent('Radio.Clear', source)
	end
end)
		...
		if input == "accept" then
			...
			if requests[identifier] then
				...
				if zPlayer then
					...
					TriggerClientEvent('chatMessage', zPlayer.source, "[SYSTEM]", {255, 0, 0}, "^0Radio freqans ^2" .. GetPlayerName(source) .. "^0 ebarat ast az: ^3" .. exports.mumble_voip:GetRadioChannel(source))
				else
