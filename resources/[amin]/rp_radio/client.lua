...
Citizen.CreateThread(function()
    ...
    Checks()
    if PlayerData.job.name == "police" or PlayerData.job.name == "ambulance" or PlayerData.job.name == "sheriff" or PlayerData.job.name == "government" or PlayerData.job.name == "doc" then
        LEO(true)
    end
                        ...
                        if newFrequency >= minFrequency and newFrequency <= radioConfig.Frequency.List[#radioConfig.Frequency.List] and newFrequency == math.floor(newFrequency) then
                            if not radioConfig.Frequency.Private[newFrequency] or radioConfig.Frequency.Access[newFrequency] then
                                ...
                                if idx ~= nil then
                                    ...
                                    TriggerServerEvent('3dme:shareDisplay', "Dastesho mibare be samte radio va freq ro avaz mikone", false)
...
AddEventHandler('esx:setJob', function(job)
    ...
    if job.name ~= "police" and job.name ~= "ambulance" and job.name ~= "sheriff" and job.name ~= "government" and job.name ~= "doc" then
        RemovePlayerAccessToFrequencies(911, 912, 913, 914, 915, 916, 917, 919)

        if PlayerData.job.name == "police" or PlayerData.job.name == "ambulance" and PlayerData.job.name == "sheriff" or PlayerData.job.name == "government" or PlayerData.job.name == "doc" then
            Radio:Remove()
        end
...
function LEO(cooldown)
    Citizen.CreateThread(function()
        ...
        GivePlayerAccessToFrequencies(911, 912, 913, 914, 915, 916, 917, 919)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            local broadcastAnimation
                if PlayerData.job.name == "police" or PlayerData.job.name == "ambulance" or PlayerData.job.name == "sheriff" or PlayerData.job.name == "doc" then
                    broadcastDictionary = "random@arrests"
                    broadcastAnimation = "generic_radio_chatter"
                else
                    broadcastDictionary = "cellphone@str"
                    broadcastAnimation = "cellphone_call_listen_a"
                end
            ...
            -- Open radio settings
            if isActivatorPressed and Radio.Enabled and Radio.Has and not isDead then
            ...
            if Radio.Open then
                ...
                if not Radio.On then
                    ...
                    if IsDisabledControlJustPressed(0, radioConfig.Controls.ToggleClicks.Key) then
                        ...
                        exports["mumble_voip"]:SetMumbleProperty("radioClicks", Radio.Clicks)
                    end
                end
            else
            ...
            if IsControlJustPressed(1, 157) and IsControlPressed(1, 21) then
                if Radio.On and PlayerData.job.name == "police" then
                    BlockWeaponWheelThisFrame()
                    radioConfig.Frequency.Current = 911
                    Radio:Add(911)
                    ESX.ShowNotification("~h~Shoma freq ra be ~g~911~w~ taghir dadid!")
                end
            end

            if IsControlJustPressed(1, 158) and IsControlPressed(1, 21) then
                if Radio.On and PlayerData.job.name == "police" then
                    BlockWeaponWheelThisFrame()
            ...
            if IsControlJustPressed(1, 160) and IsControlPressed(1, 21) then
                if Radio.On and PlayerData.job.name == "police" then
                    BlockWeaponWheelThisFrame()
            ...
            if IsControlJustPressed(1, 164) and IsControlPressed(1, 21) then
                if Radio.On and PlayerData.job.name == "police" then
                    BlockWeaponWheelThisFrame()
