...
RegisterCommand('custom', function()
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'mecano' and PlayerData.job.grade >= 2 then
...
Citizen.CreateThread(function()
	...
	while true do
		...
		local playerPed = GetPlayerPed(-1)
		if IsPedInAnyVehicle(playerPed, false) and NearAnyGarage(coords) then
							...
							ESX.TriggerServerCallback('esx_lscustom:checkStatus', function(ordered)
								...
								if not ordered then
									...
									if GetGameTimer() - lastMessage > 1000 * 60 * 5 then
										TriggerServerEvent('esx_phone:send', 'mecano', 'Salam Man be Yek Mechanic Niyaz daram, Toye Garage Mechanici montazeram', false, false)
											...
											ESX.UI.Menu.Open('question', GetCurrentResourceName(), 'Aks_For_Pay',
											{
												...
												elseif data.current.value == 'cancel' then
													if not DefaultCar then
														...
														NetworkRequestControlOfEntity(DefaultCar)
			
														while not NetworkHasControlOfEntity(DefaultCar) do
															Wait(100)
														end
...
function NearAnyGarage(coords)
	if GetDistanceBetweenCoords(coords, -338.45, -136.19, 39.01, true) < 5 or GetDistanceBetweenCoords(coords, 1175.15, 2639.22, 37.75, true) < 5 then
		return true
	else
		return false
	end
