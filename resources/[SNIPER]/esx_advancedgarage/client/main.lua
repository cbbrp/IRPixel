	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedCars', function(ownedCars)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_car', {
			...
			if data.current.value.stored then
				...
				if ESX.Game.IsSpawnPointClear({x = this_Garage.SpawnPoint.x, y = this_Garage.SpawnPoint.y, z = this_Garage.SpawnPoint.z}, 5.0) then
					SpawnVehicle(data.current.value.vehicle, data.current.value.plate)
				else
					ESX.ShowNotification("Lotfan badan emtehan konid, makane spawn por ast")
				end
			else
...
Citizen.CreateThread(function()
	...
	while true do
		Wait(sec)
		sec = 1000
		if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 221.68, -795.08, 30.87) < 10.0 then
			sec = 0
