    ...
    if type(data.number) == "number" and math.floor(data.number) == data.number then
        ...
        if PlayerData.job.name == "police" and PlayerData.job.grade < 7 then
            if IsBlackList(data.item.name) then
                ESX.ShowNotification("~h~Shoma nemitavanid aslahe haye makhsos police ra dakhel sandogh begozarid!")
                return
            end
        end

        Wait(math.random(0, 500))
        TriggerServerEvent("esx_trunk:putItem", trunkData.plate, data.item.type, data.item.name, count, trunkData.max, trunkData.myVeh, data.item.label, trunkData.label)
    end
...
RegisterNUICallback("TakeFromTrunk", function(data, cb)
    ...
    if type(data.number) == "number" and math.floor(data.number) == data.number then
        Wait(math.random(0, 500))
        TriggerServerEvent("esx_trunk:getItem", trunkData.plate, data.item.type, data.item.name, tonumber(data.number), trunkData.max, trunkData.myVeh, trunkData.label)
    end
