                    ...
                    if dist <= 1.5 then
                        ...
                        if dist <= 1.0 then
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                OpenAction(pos, Config.Items[pos["value"]], Config.Locales[pos["value"]], i)
                        	end
                        end
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1000)
        if payAmount > 0 then
            for shop = 1, #Locations do
                local blip = Locations[shop]["blip"]
...
--[[ Check what to do ]]--
OpenAction = function(action, shelf, text, shopNumber)
    if action["value"] == "checkout" then
        if payAmount > 0 and #Basket then
            CashRegister(text, shopNumber)
        else
...
--[[ Cash register menu ]]--
CashRegister = function(titel, shopNumber)
        local elements = {
        ...
        ESX.UI.Menu.Open(
            'default', GetCurrentResourceName(), 'shop_buymenushit',
            {
                title    = "Shop - " .. titel,
                align    = 'center',
                elements = elements
                        ...
                        function(data2, menu2)
                            ESX.TriggerServerCallback('99kr-shops:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    ...
                                    TriggerServerEvent('99kr-shops:Cashier', Basket, data2.current["value"], shopNumber)
                                    payAmount = 0
                                    Basket = {}
                                    menu2.close()
                                else
