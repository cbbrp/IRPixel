...
MySQL.ready(function()
	...
	for i=1, #result3 do
		...
		if ESX.Divisions[divison.owner] then
			...
			print(('es_extended: ignoring Division "%s" due to missing job!'):format(result3[i].name))
		end	
	end
	...
	for i=1, #result4 do
		...
		if ESX.Divisions[division.division_owner][division.division] then
			...
			print(('es_extended: ignoring Division grades "%s" due to missing Division on ESX.Divisions table!'):format(division.division))
		end
	end
