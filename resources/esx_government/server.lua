...
AddEventHandler('esx_government:giveItem', function(itemName)
    ...
    local xItem = xPlayer.getInventoryItem(itemName)
        ...
        if xPlayer.job.name == "government" then
            ...
            exports.BanSql:BanTarget(ource, "Attempted to get stuff from government wihtout having job", "Cheat Lua executor")
        end
end)
...
RegisterCommand("trackphone", function(source, args)
    ...
    if xPlayer.job.name == "government" or xPlayer.hasDivision("detective") then
        if xPlayer.job.grade >= 4 then
            ...
            if not args[1] then
                TriggerClientEvent('chatMessage', source, "[SYSTEM]", {255, 0, 0}, " ^0Shoma dar ghesmat shomare telephone chizi vared nakardid!")
                return
            end

            if not tonumber(args[1]) then
                TriggerClientEvent('chatMessage', source, "[SYSTEM]", {255, 0, 0}, " ^0Shoma dar ghesmat shomare faghat mojaz be vared kardan adad hastid!")
                return
            end
            ...
            MySQL.Async.fetchAll('SELECT identifier FROM users WHERE phone_number = @number',
				{
					...
					if result[1] then
                        ...
                        xPlayer = ESX.GetPlayerFromIdentifier(result[1].identifier)
                            ...
                            if xPlayer then
                                TriggerClientEvent('chatMessage', source, "[GOVERMENT]", {255, 0, 0}, " ^0Shoma ba movafaghiat aghaz be radyabi ^2" .. string.gsub(xPlayer.name, "_", " ") .. "^0 Kardid!")
                                TriggerClientEvent("esx_government:trackPhone", source, xPlayer.source)
                            else
                                TriggerClientEvent('chatMessage', source, "[GOVERMENT]", {255, 0, 0}, " ^0Shomare telephone mored nazar dar dastrest nist!")
                            end

					else
						TriggerClientEvent('chatMessage', source, "[GOVERMENT]", {255, 0, 0}, " ^0Shomare telephone vared shode vojod nadarad!")
					end
                end)
                
        else
            TriggerClientEvent('chatMessage', source, "[SYSTEM]", {255, 0, 0}, " ^0Sath dastresi shoma baraye estefade az dastor kafi nemibashad!")
        end
	else
            ...
            MySQL.Async.fetchAll('SELECT phone_number, playerName FROM users WHERE LOWER(playerName) = @name',
				{
					...
					if result[1] then
                        ...
                        TriggerClientEvent('chatMessage', source, "[GOVERMENT]", {255, 0, 0}, " ^0Shomare telephone ^2" .. result[1].playerName .. "^0: ^3" .. result[1].phone_number)

					else
						TriggerClientEvent('chatMessage', source, "[GOVERMENT]", {255, 0, 0}, " ^0Esm vared shode eshtebah ast")
					end
                end)
...
RegisterCommand("stoptrack", function(source)
    ...
    if xPlayer.job.name == "government" or xPlayer.hasDivision("detective") then
        if xPlayer.job.grade >= 4 then
            ...
            TriggerClientEvent('chatMessage', source, "[GOVERMENT]", {255, 0, 0}, " ^0Radyabi ba movafaghiat motevaghef shod!")
...
RegisterCommand("fire", function(source, args)
    ...
    if xPlayer.job.name == "government" then
        if xPlayer.job.grade >= 5 then
            ...
            if not args[1] then
                TriggerClientEvent('chatMessage', source, "[GOVERMENT]", {255, 0, 0}, " ^0Shoma dar ghesmat ID chizi vared nakardid!")
            return
            end

            if not tonumber(args[1]) then
                TriggerClientEvent('chatMessage', source, "[GOVERMENT]", {255, 0, 0}, " ^0Shoma dar ghesmat ID faghat mitavanid adad vared konid!")
            return
            end
            ...
            if xPlayer then
                ...
                if xPlayer.job.name ~= "unemployed" then
                    TriggerClientEvent('chatMessage', source, "[GOVERMENT]", {255, 0, 0}, " ^0Shoma ^2" .. string.gsub(xPlayer.name, "_", " ") .. "^0 ra az shoghl ^3 " .. xPlayer.job.name .. "^0 ^1Ekhraj^0 kardid!")
                    TriggerClientEvent('chatMessage', xPlayer.source, "[GOVERMENT]", {255, 0, 0}, " ^0Shoma tavasot dolat az shoghl khod ^1Ekhraj ^0shodid!")
                    xPlayer.setJob("unemployed", 0)
                end
            
            else
                TriggerClientEvent('chatMessage', source, "[GOVERMENT]", {255, 0, 0}, " ^0ID vared shode ghalat ast!")
            end
