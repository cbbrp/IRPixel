...
Citizen.CreateThread(function()
	...
	for k,v in pairs(Config.Government) do
	  ...
	  AddTextComponentString("Government Building")
...
AddEventHandler('esx_government:hasEnteredMarker', function(station, part, partNum)
  ...
  Citizen.CreateThread(function()
	 while true do
		  ...
		  if CurrentAction ~= nil then
			  ...
			  if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'government' then
                    ...
                    elseif CurrentAction == "menu_boss_actions" then
                        ...
                        TriggerEvent('esx_society:openBosirpixelsMenu', 'government', function(data, menu)
                            menu.close()
		  ...
		  if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'government' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'government_actions') then
...
function OpenCloakroomMenu()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
    {
        ...
        if
            data.current.value ==  grade .. '_wear'
        then
		...
		if DoesEntityExist(ped) then
			...
			ESX.TriggerServerCallback('esx_government:getGps', function(havePhone)
				...
				if havePhone then
					...
					TriggerEvent("chatMessage", "[SYSTEM]", {255, 0, 0}, "^0Signali az goshi shakhs daryaft nemishavad!")
				end
