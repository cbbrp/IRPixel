...
vehicles = {}
    ...
    RegisterNetEvent("esx_rental:deleteVehicle")
	AddEventHandler("esx_rental:deleteVehicle",function(NetId)
		...
		if DoesEntityExist(vehicle) then
			...
			if not IsAnyPedInVehicle(vehicle) then
				...
				if not DoesEntityExist(vehicle) then 
					TriggerServerEvent('esx_rental:deleteVehicle', NetId)
				end
...
function SpawnVehicle(request)
	...
	-- / Network related shit
	local NetID = NetworkGetNetworkIdFromEntity(vehicle)
	TriggerServerEvent('esx_rental:addVehicleShit', NetID)
...
function checkVehicles()
	...
	if TableLength(vehicles) > 0 then
		...
		for k,v in pairs(vehicles) do
			...
			local vehicle = NetworkGetEntityFromNetworkId(v.id)
			if DoesEntityExist(vehicle) then

				if IsAnyPedInVehicle(vehicle) then
					TriggerServerEvent('esx_rental:updateTime', v.id)
				end
