...
AddEventHandler('esx_jobs:action', function(job, zone)
	...
	elseif zone.Type == "work" then
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			TriggerServerEvent('esx_jobs:starirtpixelWork', zone.Item)
		end
							...
							if playerPed == driverPed then
								...
								for i=1, #myPlate, 1 do
									if myPlate[i] == plate then
										...
										TriggerServerEvent('esx_jobs:cairpixelution', "give_back", giveBack, 0, 0)
...
AddEventHandler('esx_jobs:hasExitedMarker', function(zone)
	TriggerServerEvent('esx_jobs:stoirppixelWork')
	hintToDisplay = "no hint to display"
	menuIsShowed = false
	hintIsShowed = false
	isInMarker = false
...
function spawnVehicle(spawnPoint, vehicle, vehicleCaution)
	hintToDisplay = 'no hint to display'
	hintIsShowed = false
	TriggerServerEvent('esx_jobs:cairpixelution', 'take', vehicleCaution, spawnPoint, vehicle)
end
