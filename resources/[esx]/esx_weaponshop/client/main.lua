...
function OpenBuyLicenseMenu(zone)
    ...
    ESX.UI.Menu.Open('question', GetCurrentResourceName(), 'not_license',
    {
        title 	 = 'Ekhtarie',
        align    = 'center',
        question = "Shoma mojavez aslahe nadarid baraye daryaft mojavez bayad be pasgah police moraje konid!",
        elements = {
            {label = 'Bashe', value = 'yes'},
        }
    }, function(data, menu)
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				if CurrentAction == 'shop_menu' then
					ESX.TriggerServerCallback('esx_weaponshop:CheckLicenses', function(user)
						if not user then
							ESX.ShowNotification('~h~Khatayi dar gereftan license haye shoma pish amad lotfan be developer etelaa dahid!')
							return
						end
