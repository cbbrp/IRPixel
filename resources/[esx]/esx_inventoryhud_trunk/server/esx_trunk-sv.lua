...
AddEventHandler("esx_trunk:getItem",function(plate, type, item, count, max, owned)
            ...
            for i = 1, #coffre, 1 do
              if coffre[i].name == item then
                if (coffre[i].count >= count and count > 0) then
                  ...
                  local info = {source = xPlayer.source, icname = xPlayer.name, name = item, type = "Bardasht", plate = plate, count = "1"}
                   exports.sniperlog:TrunkLog(info)
    ...
    if type == "item_weapon" then
      TriggerEvent("esx_trunk:getSharedDataStore", plate, function(store)
          ...
          local info = {source = xPlayer.source, icname = xPlayer.name, name = weaponName, type = "Bardasht", plate = plate, count = "1"}
          exports.sniperlog:TrunkLog(info)
...
AddEventHandler("esx_trunk:putItem", function(plate, type, item, count, max, owned)
        ...
        TriggerEvent("esx_trunk:getSharedDataStore", plate, function(store)
            ...
            if (getTotalInventoryWeight(plate) + (getItemWeight(item) * count)) > max then
              ...
              if xPlayer.getInventoryItem(item).count > 0 then
                ...
                local info = {source = xPlayer.source, icname = xPlayer.name, name = item, type = "Gozasht", plate = plate, count = count}
                exports.sniperlog:TrunkLog(info)
              end
    ...
    if type == "item_weapon" then
      local aweapon = xPlayer.hasWeapon(item)
      if aweapon then
        TriggerEvent("esx_trunk:getSharedDataStore", plate, function(store)
            local storeWeapons = store.get("weapons")
            ...
            if (getTotalInventoryWeight(plate) + (getItemWeight(item))) > max then
              ...
              store.set("weapons", storeWeapons)
                ...
                MySQL.Async.execute(
                  ...
                  local info = {source = xPlayer.source, icname = xPlayer.name, name = item, type = "Gozasht", plate = plate, count = "1"}
                  exports.sniperlog:TrunkLog(info)

            end
          end)
      else
        exports.BanSql:BanTarget(xPlayer.source, "Tried to duplicate guns in the trunk", "Duplicate Weapons")
      end
