...
RegisterServerEvent('esx_ambulancejob:reirpixelvive')
AddEventHandler('esx_ambulancejob:reirpixelvive', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'ambulance' then
		xPlayer.addMoney(Config.ReviveReward)
		TriggerClientEvent('esx_ambulancejob:reirpixelvive', target)
	else
	...
	if xPlayer.get('aduty') then
		...
		if args[1] ~= nil then
			if GetPlayerName(tonumber(args[1])) ~= nil then
				...
				TriggerClientEvent('esx_ambulancejob:reirpixelvive', tonumber(args[1]))
			end
		else
			TriggerClientEvent('esx_ambulancejob:reirpixelvive', source)
		end
...
function healCheck()
	for k,v in pairs(heals) do
		if os.time() - v.time >= 120 then
			...
			if xPlayer then TriggerClientEvent('chatMessage', xPlayer.source, "[SYSTEM]", {255, 0, 0}, "^0Darkhast ^2heal ^0shoma ^1monghazi^0 shod!") end
			heals[k] = nil
		end
	end
