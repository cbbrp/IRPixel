...
AddEventHandler('esx_duty:hasEnteredMarker', function (zone)
  ...
  elseif zone == 'PoliceDuty' then
    ...
    CurrentActionMsg  = _U('duty')
    CurrentActionData = {}
  elseif zone == 'MechanicDuty' or zone == "MechanicDuty2" then
    CurrentAction     = 'mechanic_duty'
    CurrentActionMsg  = _U('duty')
    CurrentActionData = {}
  elseif zone == 'DocDuty' then
    CurrentAction     = 'doc_duty'
    CurrentActionMsg  = _U('duty')
    CurrentActionData = {}
  elseif zone == 'TaxiDuty' then
    CurrentAction     = 'taxi_duty'
    CurrentActionMsg  = _U('duty')
    CurrentActionData = {}
  elseif zone == 'CoffeeDuty' then
    CurrentAction     = 'coffee_duty'
    CurrentActionMsg  = _U('duty')
    CurrentActionData = {}
  ...
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0, Keys['E']) then
        ...
        elseif CurrentAction == 'doc_duty' then

          if PlayerData.job.name == 'doc' or PlayerData.job.name == 'offdoc' then
              TriggerServerEvent('duty:doc')
              
              if PlayerData.job.name == 'doc' then
                sendNotification(_U('offduty'), 'success', 2500)
            ...
            sendNotification('shoma DOC nistid', 'error', 5000)
            Wait(1000)
          end
        ...
        elseif CurrentAction == 'taxi_duty' then
          ...
          if PlayerData.job.name == 'taxi' or PlayerData.job.name == 'offtaxi' then
            ...
            sendNotification("Shoma taxi nistid!", 'error', 5000)
            Wait(1000)
          end

        elseif CurrentAction == 'coffee_duty' then

          if PlayerData.job.name == 'coffee' or PlayerData.job.name == 'offcoffee' then
            TriggerServerEvent('duty:coffee')
            ...
            sendNotification("Shoma ozv cafe nistid!", 'error', 5000)
            Wait(1000)
          end
