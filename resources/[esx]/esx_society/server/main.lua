...
RegisterServerEvent('esx_society:putVehicleirpixelInGarage')
AddEventHandler('esx_society:putVehicleirpixelInGarage', function(societyName, vehicle)
...
ESX.RegisterServerCallback('esx_society:getEmployees', function(source, cb, society)
	MySQL.Async.fetchAll('SELECT name, playerName, identifier, job, job_grade FROM users WHERE job = @job OR job = @off ORDER BY job_grade DESC', {
		['@job'] = society,
		['@off'] = "off" .. society
	}, function (result)
		local employees = {}
		...
		for i=1, #result, 1 do
			table.insert(employees, {
				name       = result[i].playerName .. " ("  .. result[i].name .. ")",
...
ESX.RegisterServerCallback('esx_society:getOnlirpixelinePlayers', function(source, cb)
	local xPlayers = ESX.GetPlayers()
	local players  = {}
