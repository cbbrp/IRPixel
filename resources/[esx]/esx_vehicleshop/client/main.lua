local Keys = {
  ...
  function OpenShopMenu()
	  ...
	  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		  ...
		  if ESX.PlayerData.gang.name ~= 'nogang' then
			elements = {
				{label = 'Kharid mashin baraye gang', value = 'buyforgang'},
				{label = 'Kharid mashin baraye khod', value = 'buyformyself'}
			}
		  else
			elements = {
					...
					if hasEnoughMoney then
						...
						ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
							...
							if Config.EnableOwnedVehicles then
								TriggerServerEvent('esx_vehicleshop:setVehicle2Owned', vehicleProps)
							end
				...
				ESX.TriggerServerCallback('esx_vehicleshop:buyVehicle', function (hasEnoughMoney)
					if hasEnoughMoney then
						...
						ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function (vehicle)
							...
							vehicleProps.plate = newPlate
							SetVehicleNumberPlateText(vehicle, newPlate)
							if data2.current.value == 'buyformyself' then
								TriggerServerEvent('esx_vehicleshop:setVehicle2Owned', vehicleProps)

								ESX.ShowNotification(_U('vehicle_purchased'))
							elseif data2.current.value == 'buyforgang' then
								TriggerServerEvent('esx_vehicleshop:setVehicle2Owned', vehicleProps, ESX.PlayerData.gang.name)

								ESX.ShowNotification("Shoma In Mashin Ro Baraye Gang Khod Kharidid")
							end
						end)
	  ...
	  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
			  ...
			  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
				  ...
				  if amount == nil then
					  ...
					  if closestPlayer == -1 or closestDistance > 3.0 then
						  ...
						  TriggerServerEvent('esx_biirpixellling:sendBill', GetPlayerServerId(closestPlayer), 'society_cardealer', _U('car_dealer'), tonumber(data2.value))
					  end
				  end
			  end, function (data2, menu2)
				  menu2.close()
		  ...
		  elseif action == 'set_vehicle_owner_sell' then
			  ...
			  if closestPlayer == -1 or closestDistance > 3.0 then
				  ...
				  if Config.EnableOwnedVehicles then
					  TriggerServerEvent('esx_vehicleshop:setVehicleOwnedirpixelPlayerId', GetPlayerServerId(closestPlayer), vehicleProps)
			  ...
			  if closestPlayer == -1 or closestDistance > 3.0 then
				  ...
				  ESX.TriggerServerCallback('esx:getOtherPlayerData', function (xPlayer)
					  ...
					  if Config.EnableSocietyOwnedVehicles then
						  TriggerServerEvent('esx_vehicleshop:setVehicleOwnedirpixelSociety', xPlayer.job.name, vehicleProps)
						  ESX.ShowNotification(_U('vehicle_set_owned', vehicleProps.plate, GetPlayerName(closestPlayer)))
					  else
						  ESX.ShowNotification(_U('vehicle_sold_to', vehicleProps.plate, GetPlayerName(closestPlayer)))
					  end
  ...
  function OpenBossActionsMenu()
	  ...
	  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		  ...
		  if data.current.value == 'boss_actions' then
			  TriggerEvent('esx_society:openBosirpixelsMenu', 'cardealer', function(data2, menu2)
				  menu2.close()
			  end)
		  ...
		  if CurrentAction == nil then
			  ...
			  if IsControlJustReleased(0, Keys['E']) then
				  ...
				  elseif CurrentAction == 'resell_vehicle' then
					...
					if GetPedInVehicleSeat(CurrentActionData.vehicle, -1) == GetPlayerPed(-1) then
						...
						ESX.ShowNotification("~h~Shoma ranande in mashin nistid!")
					end	  
  ...
  Citizen.CreateThread(function()
	AddTextEntryByHash(GetHashKey("s65amg"), "Benz S65")
	AddTextEntryByHash(GetHashKey("c63"), "Benz C63")
	AddTextEntryByHash(GetHashKey("urus"), "Urus")
	AddTextEntryByHash(GetHashKey("rmodm4gts"), "BMW M4")
	AddTextEntryByHash(GetHashKey("rmodx6"), "BMW X6")
	AddTextEntryByHash(GetHashKey("hevo"), "Lamborgini Hurcan")
	AddTextEntryByHash(GetHashKey("g65amg"), "Benz GClass")
	AddTextEntryByHash(GetHashKey("rmodlp750"), "Lamborgini Aventador")
	AddTextEntryByHash(GetHashKey("chiron17"), "Bugati Chiron")
	AddTextEntryByHash(GetHashKey("lex570"), "Lexus 570")
	AddTextEntryByHash(GetHashKey("raptor150"), "Ford Raptor")
	AddTextEntryByHash(GetHashKey("laferrari17"), "La Ferrari")
	AddTextEntryByHash(GetHashKey("amggtsprior"), "Benz SLS")
	AddTextEntryByHash(GetHashKey("yzfr6"), "Yamaha")
	AddTextEntryByHash(GetHashKey("str20"), "Porche STR")
	AddTextEntryByHash(GetHashKey("mb300sl"), "Benz 300SL")
	AddTextEntryByHash(GetHashKey("cls2015"), "Benz 563")
	AddTextEntryByHash(GetHashKey("16challenger"), "Dodge Challenger")
end)