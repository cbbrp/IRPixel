...
RegisterCommand('forceloadvehicles', function(source)
...
RegisterServerEvent('esx_vehicleshop:setVehicleOwnedirpixelPlayerId')
AddEventHandler('esx_vehicleshop:setVehicleOwnedirpixelPlayerId', function (playerId, vehicleProps, vehtype)
	local _source = source
	...
	if xPlayer.permission_level >= 9 then
		...
		{
			['@owner']   = GetPlayerIdentifier(playerId),
			['@plate']   = vehicleProps.plate,
			['@vehicle'] = json.encode(vehicleProps),
			['@type'] = vehtype
		}, function (rowsChanged)
...
-- RegisterServerEvent('esx_vehicleshop:setVehicleOwnedirpixelSociety')
-- AddEventHandler('esx_vehicleshop:setVehicleOwnedirpixelSociety', function (society, vehicleProps)
-- 	local _source = source
-- 	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function (source, cb, vehicleModel)
	...
	if xPlayer.money >= vehicleData.price then
		cb(true)
	else
		cb(false)
	end
