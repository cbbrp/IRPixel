...
AddEventHandler('esx_holdupbank:toofar', function(robb)
	...
	for k,v in pairs(exports.esx_scoreboard:GetJob("police")) do
		...
		TriggerClientEvent('esx_holdupbank:killblip', k)
	end
...
AddEventHandler('esx_holdupbank:rob', function(robb)
	...
	if Banks[robb] then
		...
		if (os.time() - bank.lastrobbed) < 14400 and bank.lastrobbed ~= 0 then
			...
			TriggerClientEvent('esx:showNotification', source, _U('already_robbed') .. (14400 - (os.time() - bank.lastrobbed)) .. _U('seconds'))
			return
		end
		...
		if rob == false then
			 ...
			 if xPlayer.getInventoryItem('blowtorch').count >= 1 then
				...
				if(exports.esx_scoreboard:GetCounts("police") >= Config.NumberOfCopsRequired) then
					xPlayer.removeInventoryItem('blowtorch', 1)

					rob = true
					for k,v in pairs(exports.esx_scoreboard:GetJob("police")) do
						TriggerClientEvent('chatMessage', k, 'NEWS', {255, 0, 0}, "Robbery in progress at ^2" .. bank.nameofbank)
						TriggerClientEvent('esx:showNotification', k, _U('rob_in_prog') .. bank.nameofbank)
						TriggerClientEvent('esx_holdupbank:killblip', k)							
						TriggerClientEvent('esx_holdupbank:setblip', k, Banks[robb].position)
					end
					...
					SetTimeout(600000, function()
						...
						if(robbers[savedSource])then
							...
							if(xPlayer)then
								...
								local info = {source= xPlayer.source, type = "Bank", action = "Completed", location = bank.nameofbank, amount = bank.reward}
				...
				SetTimeout(20000, function()
					...
					if(robbers[savedSource])then
						...
						if(xPlayer)then
							...
							for k,v in pairs(exports.esx_scoreboard:GetJob("police")) do
								TriggerClientEvent('esx:showNotification', k, _U('bombplanted_at') .. bank.nameofbank)
							end
