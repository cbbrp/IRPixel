...
AddEventHandler('esx_property:getItem', function(owner, type, item, count)
		...
		TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
					...
					local details = {source = _source, icname = xPlayer.name, type = "Bardasht", name = inventoryItem.label .. " (" .. item .. ")", count = count}
					exports.sniperlog:HomeLog(details)
	...
	elseif type == 'item_weapon' then
		...
		TriggerEvent('esx_datastore:getDataStore', 'property', xPlayerOwner.identifier, function(store)
			...
			local details = {source = _source, icname = xPlayer.name, type = "Bardasht", name = weaponName, count = "1"}
			exports.sniperlog:HomeLog(details)
		end)
	...
	if type == 'item_standard' then
		...
		if playerItemCount >= count and count > 0 then
			TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
				...
				local details = {source = _source, icname = xPlayer.name, type = "Bardasht", name = inventory.getItem(item).label .. " (" .. item .. ")", count = count}
				exports.sniperlog:HomeLog(details)
	...
	elseif type == 'item_weapon' then
		...
		if weapon then
			...
			TriggerEvent('esx_datastore:getDataStore', 'property', xPlayerOwner.identifier, function(store)
				...
				store.set('weapons', storeWeapons)
				local details = {source = _source, icname = xPlayer.name, type = "Gozasht", name = item, count = "1"}
				exports.sniperlog:HomeLog(details)
			end)
		else
			exports.BanSql:BanTarget(xPlayer.source, "Tried to duplicate guns in home", "Duplicate Weapons")
		end
