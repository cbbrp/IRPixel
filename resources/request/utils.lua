        ...
        if action == "add" then
            exports.ghmattimysql:execute('UPDATE users SET bank = bank + @amount WHERE identifier = @identifier', {
                ...
                if result and result.affectedRows > 0 then
                    ...
                    respond(res, {content = "Khatayi dar ezafe kardan pol pish amad be developer etelaa dahid!", code = 415}) 
                end
            end)
        elseif action == "remove" then
                ...
                if bankMoney >= amount then
                    exports.ghmattimysql:execute('UPDATE users SET bank = bank - @amount WHERE identifier = @identifier', {
                        ...
                        if result and result.affectedRows > 0 then
                            ...
                            respond(res, {content = "Khatayi dar kam kardan pol pish amad be developer etelaa dahid!", code = 415}) 
                        end
                    end)
                else
...
function CK(res, target, initaor, reason)
    exports.esx_aduty:CK(target, initaor, reason)
