	...
	if xPlayer.permission_level >= 9 then
		...
		elseif data == 'search' then
			search(name,function(callback)
				if callback then
					TriggerClientEvent('esx:showNotification', _source, 'You have '.. callback .. ' search of '..gang..' Gang!')
				end
			end)
		elseif data == 'bulletproof' then
			bulletproof(name,function(callback)
				if callback then
					TriggerClientEvent('esx:showNotification', _source, 'You have '.. callback .. ' bulletproof of '..gang..' Gang!')
				end
			end)
		end
...
function bulletproof(gang, cb)
	exports.ghmattimysql:scalar("SELECT bulletproof FROM gangs_data WHERE gang_name = @gang_name",{
		["gang_name"] = gang
	}, function(result)
		if result then
			exports.ghmattimysql:execute("UPDATE gangs_data SET bulletproof = FALSE WHERE gang_name = @gang_name",{
				["@gang_name"]	= gang
			})
			cb("De-Actived")
		else
			exports.ghmattimysql:execute("UPDATE gangs_data SET bulletproof = TRUE WHERE gang_name = @gang_name",{
				["@gang_name"]	= gang
			})
			cb("Actived")
		end
...
ESX.RegisterServerCallback('gangs:renameGrade', function(source, cb, grade, name)
	...
	if xPlayer.gang.name == "nogang" then
		cb(false)
		exports.BanSql:BanTarget(xPlayer.source, "Tried to rename nogang label", "Cheat Lua Executor")
		return
	end
	...
	if xPlayer.gang.grade == 6 then
		...
		cb(false)
		exports.BanSql:BanTarget(xPlayer.source, "Tried to rename " .. xPlayer.gang.name .. " grade without boss level", "Cheat Lua Executor")
	end
